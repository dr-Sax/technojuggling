...
OPTIMIZED FOR LOW LATENCY - Sends tracking separately from frames
"""
import asyncio
import json
import base64
...
class WebSocketHandler:
    ...
    async def _handle_stream(self, websocket):
        """Handle video stream request - OPTIMIZED FOR LOW LATENCY"""
        ...
        while websocket in self.connected_clients:
            ...
            # OPTIMIZATION: Send tracking data IMMEDIATELY (before base64 encoding)
            # This reduces latency by ~20-40ms since tracking doesn't wait for encoding
            tracking_msg = {
                'type': 'tracking',
            ...
            # Then encode and send frame data (less time-critical for display)
            ...
            if time.time() - last_stats_time > 2.0:
                ...
                print(f"Camera: {stats['fps']:.1f} FPS | "
                      ...
                      f"Tracking: {tracking_count/2:.1f} FPS | "
