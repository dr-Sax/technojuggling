 ...
 * Mesh mode: all-to-all, Sequential mode: loop
 * 
 * Updated to use MaterialFactory and plugin system
 */
...
export class ConnectionLines extends GeometryBase {
  ...
  createGeometry(id, { p1, p2 }) {
    // Use GeometryPrimitives for tube creation
    ...
    // Use MaterialFactory for material
  ...
  updateGeometry(id, { p1, p2 }) {
    ...
    if (moved) this.add(id, { p1, p2 }); // Recreate
  }
  ...
  setConfig(config) {
    ...
    if (needsRecreate) {
      // Recreate all lines with new width
